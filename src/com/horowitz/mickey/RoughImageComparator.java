...
public class RoughImageComparator implements ImageComparator {
	 ...
	 *          TODO
	 */
	public RoughImageComparator(double roughness, int precision) {
	 ...
	 * 
	 * @see
	 * com.horowitz.mickey.IImageComparator#compare(java.awt.image.BufferedImage,
	 ...
	 * 
	 * @see
	 * com.horowitz.mickey.IImageComparator#compareOld(java.awt.image.BufferedImage
	 * , java.awt.image.BufferedImage, com.horowitz.mickey.Pixel[])
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.horowitz.mickey.IImageComparator#findSimilarities(java.awt.image.
	 * BufferedImage, java.awt.image.BufferedImage, com.horowitz.mickey.Pixel[])
	 */
	@Override
	 ...
	 * @see
	 * com.horowitz.mickey.IImageComparator#compare(java.awt.image.BufferedImage,
	 * com.horowitz.mickey.Pixel[], java.awt.Color[])
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.horowitz.mickey.IImageComparator#compareInt(java.awt.image.BufferedImage
	 * , com.horowitz.mickey.Pixel[], java.awt.Color[])
	 */
	@Override
	...
	/*
	 * public Pixel compareLetter(BufferedImage image, Letter letter, Color[]
	 * colors) { return compareLetter(image, 0, 0, -1, -1, letter, colors); }
	 * 
	 * public Pixel compareLetter(BufferedImage image, int startY, int startX, int
	 * maxOffsetY, int maxOffsetX, Letter letter, Color[] colors) { int stX =
	 * startX; int stY = startY; int maxY = startY + maxOffsetY; int maxX = startX
	 * + maxOffsetX; if (maxX > image.getWidth() - letter.getWidth()) { maxX =
	 * image.getWidth() - letter.getWidth(); } if (maxY > image.getHeight() -
	 * letter.getHeight()) { maxY = image.getHeight() - letter.getHeight(); } if
	 * (maxOffsetY < 0) { maxY = image.getHeight() - letter.getHeight(); } else {
	 * stY = startY -maxOffsetY > 0 ? startY - maxOffsetY : startY; } if
	 * (maxOffsetX < 0) { maxX = image.getWidth() - letter.getWidth(); } else {
	 * stX = startX - maxOffsetX > 0? startX - maxOffsetX : startX; }
	 * 
	 * for (int x = stX; x < maxX; x++) { for (int y = stY ; y < maxY; y++) { //
	 ...
	 * assert letter.getPixelMask() != null : "letter mask null" + letter; assert
	 * letter.getPixelMask().length > 0 : "letter mask empty" + letter;
	 * //System.err.println(letter + " y=" + y + ", x= "+ x + ", width=" +
	 * letter.getWidth() + ", height=" + letter.getHeight() + "   image size is: "
	 * + image.getWidth() + " x " + image.getHeight()); if
	 * (compareLetterInt(subimage, letter.getPixelMask(), colors)) { return new
	 * Pixel(y, x); } } catch (RuntimeException e1) { //e1.printStackTrace();
	 * System.err.println(letter + " y=" + y + ", x= "+ x + ", width=" +
	 * letter.getWidth() + ", height=" + letter.getHeight() + "   image size is: "
	 * + image.getWidth() + " x " + image.getHeight()); } } } return new Pixel(-1,
	 * -1); }
	 * 
	 * public boolean compareLetterInt(BufferedImage image, Pixel[] mask, Color[]
	 * colors) { assert (mask != null) && (mask.length > 0); for (int i = 0; i <
	 * mask.length; i++) { final int rgb1 = image.getRGB(mask[i].x, mask[i].y);
	 * final Color c1 = new Color(rgb1); if (mask[i].weight < 0) { // negative
	 * color boolean found = false; for (int j = 0; (j < colors.length) && !found;
	 * j++) { final Color c2 = colors[j]; if (((c1.getRed() != c2.getRed()) ||
	 * (c1.getGreen() != c2.getGreen()) || (c1.getBlue() != c2.getBlue()))) {
	 * found = true; } } if (!found) { return false; } } else { boolean found =
	 * false; for (int j = 0; (j < colors.length) && !found; j++) { final Color c2
	 * = colors[j]; if (((c1.getRed() == c2.getRed()) && (c1.getGreen() ==
	 * c2.getGreen()) && (c1.getBlue() == c2.getBlue()))) { found = true; } } if
	 ...
	 * 
	 * @see
	 * com.horowitz.mickey.IImageComparator#findPoint(java.awt.image.BufferedImage
	 * , com.horowitz.mickey.Pixel[], java.awt.Color[])
	 */
	@Override
